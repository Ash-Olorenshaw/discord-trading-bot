...
async def on_message(message):
    ...
    player_line = check_list_items_contain(lines, message.author.name)
    ...
    if message.content.lower()[:2] == "-s" and player_line == -1 and message.content != "-s enrol":
        ...
        await gen_user_inv(message, lines, priceFile, refFile, networths)
        ...
        if times < 0:
            ...
            if not times.is_integer():
                ...
                if not itemCheck == -1:
                    userItems = lines[player_line].split("-")
                    ...
                    if float(userItems[1]) >= (float(itemVals[-2]) * times):
                        if not check_list_items_contain(available_items, buyItem) == -1:
                            ...
                            lines[player_line] = convert_list_to_string(userItems)
        ...
        if not netLine == -1 and gen_networth(message.author.name, lines, priceFile) < defaultNetworth:
            ...
            usersFile = open(userFile, "r+")
            lines[player_line] = message.author.name + default_items + "\n"
            usersFile.seek(0)
            usersFile.truncate(0)
            for line in lines:
                usersFile.write(line)
